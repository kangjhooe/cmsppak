...
@section('title', 'Widget Beranda - ' . $schoolName)

@section('content')
@php
    $hasFilters = request()->filled('search') || request()->filled('status') || request()->filled('tipe');
@endphp
    ...
    <div class="w-full bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 relative overflow-hidden">
        ...
        <div class="relative w-full px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 max-w-7xl mx-auto">
                <div>
                    <p class="text-cyan-100 text-sm font-medium mb-2">Konten Website</p>
                    <h1 class="text-3xl sm:text-4xl font-bold text-white tracking-tight">Widget Beranda</h1>
                    <p class="mt-2 text-cyan-50/90 text-base max-w-xl">Atur widget yang tampil di kanan konten beranda. Waktu sholat dan kalender Hijriah opsional — cocok untuk madrasah atau pesantren, bisa dimatikan untuk sekolah umum.</p>
                </div>
                <a href="{{ route('admin.homepage-widgets.create') }}"
                   class="inline-flex items-center justify-center gap-2 self-start md:self-auto px-5 py-2.5 rounded-xl bg-white text-teal-700 font-semibold text-sm shadow-lg shadow-teal-900/20 hover:bg-teal-50 transition-colors">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-4 hover:shadow-md hover:border-teal-200 transition-all duration-200">
                <div class="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-teal-100 flex items-center justify-center">
                    <i class="fas fa-th-large text-teal-600 text-lg sm:text-xl"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm text-slate-500 font-medium truncate">Total Widget</p>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-4 hover:shadow-md hover:border-emerald-200 transition-all duration-200">
                <div class="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-emerald-100 flex items-center justify-center">
                    <i class="fas fa-toggle-on text-emerald-600 text-lg sm:text-xl"></i>
                </div>
                <div class="min-w-0">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="px-5 sm:px-6 py-5 border-b border-slate-100">
                <h2 class="text-lg font-semibold text-slate-900">Daftar Widget</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ $widgets->total() }} widget terdaftar</p>
            </div>
            ...
            <div class="p-5 sm:p-6">
                <form method="GET" action="{{ route('admin.homepage-widgets.index') }}" class="mb-6">
                    <div class="flex flex-col gap-3">
                        <div class="relative">
                            ...
                            @if(request('search'))
                                <a href="{{ route('admin.homepage-widgets.index', request()->except('search', 'page')) }}"
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <div class="relative flex-1 sm:max-w-[200px]">
                                <select name="tipe" id="tipe"
                                    ...
                                    @foreach(\App\Models\HomepageWidget::TYPES as $key => $label)
                    ...
                    @if($hasFilters)
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            ...
                            @if(request('search'))
                                <span class="inline-flex items-center h-7 px-2.5 rounded-lg bg-teal-50 text-teal-700 text-xs font-medium border border-teal-100">
                                    “{{ Str::limit(request('search'), 24) }}”
                                </span>
                            @endif
                            @if(request('status'))
                                <span class="inline-flex items-center h-7 px-2.5 rounded-lg bg-cyan-50 text-cyan-700 text-xs font-medium border border-cyan-100">
                            ...
                            @if(request('tipe'))
                                <span class="inline-flex items-center h-7 px-2.5 rounded-lg bg-sky-50 text-sky-700 text-xs font-medium border border-sky-100">
                                    {{ \App\Models\HomepageWidget::TYPES[request('tipe')] ?? request('tipe') }}
                                </span>
                            @endif
                            <span class="text-xs text-slate-400 sm:ml-auto">{{ $widgets->total() }} hasil</span>
                        </div>
                    @endif
                </form>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-4 py-4 whitespace-nowrap text-center">
                                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 text-sm font-semibold text-slate-600">
                                        {{ $widgets->firstItem() + $loop->index }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-slate-800">{{ $widget->urutan }}</span>
                                </td>
                                <td class="px-4 py-4">
                                    <p class="text-sm font-semibold text-slate-900">{{ $widget->judul }}</p>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border bg-cyan-50 text-cyan-700 border-cyan-100">
                                        {{ $widget->tipe_label }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('admin.homepage-widgets.edit', $widget) }}"
                                           class="inline-flex items-center justify-center w-9 h-9 rounded-lg text-teal-600 hover:bg-teal-50 transition-colors" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.homepage-widgets.destroy', $widget) }}" method="POST" class="inline" onsubmit="return confirm('Hapus widget ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="mx-auto w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center mb-4">
                                        <i class="fas fa-th-large text-2xl text-slate-400"></i>
                                    </div>
                                    <h3 class="text-lg font-semibold text-slate-900 mb-1">
                                        {{ $hasFilters ? 'Tidak ada widget yang cocok' : 'Belum ada widget' }}
                                    </h3>
                                    <p class="text-sm text-slate-500 mb-5">
                                        {{ $hasFilters ? 'Coba ubah kata kunci atau reset filter.' : 'Mulai dengan menambahkan widget pertama.' }}
                                    </p>
                                    @if($hasFilters)
                                        <a href="{{ route('admin.homepage-widgets.index') }}"
                                    ...
                                    @else
                                        <a href="{{ route('admin.homepage-widgets.create') }}"
                                           class="inline-flex items-center gap-2 h-10 px-5 rounded-xl bg-teal-600 text-white text-sm font-semibold hover:bg-teal-700">
                ...
                <div class="mt-5 pt-5 border-t border-slate-100">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <p class="text-sm text-slate-500">
                            Menampilkan
                            <span class="font-semibold text-slate-800">{{ $widgets->firstItem() ?? 0 }}</span>
                            –
                            <span class="font-semibold text-slate-800">{{ $widgets->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-semibold text-slate-800">{{ $widgets->total() }}</span>
                        </p>
