    ...
    <div class="w-full bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 relative overflow-hidden">
        ...
        <div class="relative w-full px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 max-w-7xl mx-auto">
                <div>
                    <p class="text-blue-100 text-sm font-medium mb-2">Konten Website</p>
                    <h1 class="text-3xl sm:text-4xl font-bold text-white tracking-tight">Manajemen Fitur</h1>
                    <p class="mt-2 text-blue-50/90 text-base max-w-xl">Kelola fitur “Mengapa Memilih Kami?” di halaman beranda.</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-4 hover:shadow-md hover:border-indigo-200 transition-all duration-200">
                <div class="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-indigo-100 flex items-center justify-center">
                    <i class="fas fa-check-circle text-indigo-600 text-lg sm:text-xl"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm text-slate-500 font-medium truncate">Total Fitur</p>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="px-5 sm:px-6 py-5 border-b border-slate-100">
                <h2 class="text-lg font-semibold text-slate-900">Daftar Fitur</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ $features->total() }} fitur terdaftar</p>
            </div>
            ...
            <div class="p-5 sm:p-6">
                <form method="GET" action="{{ route('admin.features.index') }}" class="mb-6">
                    <div class="flex flex-col gap-3">
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}"
                                   placeholder="Cari judul atau deskripsi fitur..."
                            ...
                            @if(request('search'))
                                <a href="{{ route('admin.features.index', request()->except('search', 'page')) }}"
                    ...
                    @if($hasFilters)
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            ...
                            @endif
                            <span class="text-xs text-slate-400 sm:ml-auto">{{ $features->total() }} hasil</span>
                        </div>
                    @endif
                </form>
                        ...
                        <tbody class="bg-white divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-slate-800">{{ $feature->urutan }}</span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="w-11 h-11 rounded-xl flex items-center justify-center {{ $warnaIcon[$feature->warna] ?? 'bg-slate-100 text-slate-600' }}">
                                        <i class="{{ $feature->icon }} text-lg"></i>
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                    <p class="text-sm font-semibold text-slate-900">{{ $feature->judul }}</p>
                                </td>
                                <td class="px-4 py-4">
                                    <p class="text-sm text-slate-500 max-w-xs line-clamp-2">{{ Str::limit($feature->deskripsi, 80) }}</p>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border {{ $warnaBadge[$feature->warna] ?? 'bg-slate-50 text-slate-600 border-slate-100' }}">
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-16 text-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-slate-900 mb-1">
                                        {{ $hasFilters ? 'Tidak ada fitur yang cocok' : 'Belum ada fitur' }}
                                    </h3>
                                    <p class="text-sm text-slate-500 mb-5">
                                        {{ $hasFilters ? 'Coba ubah kata kunci atau reset filter.' : 'Mulai dengan menambahkan fitur pertama.' }}
                                    </p>
                                    @if($hasFilters)
                                        <a href="{{ route('admin.features.index') }}"
                ...
                <div class="mt-5 pt-5 border-t border-slate-100">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <p class="text-sm text-slate-500">
                            Menampilkan
                            <span class="font-semibold text-slate-800">{{ $features->firstItem() ?? 0 }}</span>
                            –
                            <span class="font-semibold text-slate-800">{{ $features->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-semibold text-slate-800">{{ $features->total() }}</span>
                        </p>
