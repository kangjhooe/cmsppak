...
@section('title', 'Hero Slider - ' . $schoolName)
...
<div class="bg-slate-50 pb-10">
    <!-- Banner: pertahankan gradasi emerald → green → teal -->
        ...
        <div class="relative w-full px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 max-w-7xl mx-auto">
                <div>
                    ...
                    <h1 class="text-3xl sm:text-4xl font-bold text-white tracking-tight">Hero Slider</h1>
                    <p class="mt-2 text-emerald-50/90 text-base max-w-xl">Kelola gambar slider full halaman di beranda.</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-4 hover:shadow-md hover:border-emerald-200 transition-all duration-200">
                <div class="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-emerald-100 flex items-center justify-center">
                    <i class="fas fa-images text-emerald-600 text-lg sm:text-xl"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs sm:text-sm text-slate-500 font-medium truncate">Total Slide</p>
                    <p class="text-2xl sm:text-3xl font-bold text-slate-900 leading-tight">{{ $stats['total'] }}</p>
                </div>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex items-center gap-4 hover:shadow-md hover:border-green-200 transition-all duration-200">
                <div class="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-green-100 flex items-center justify-center">
                    <i class="fas fa-toggle-on text-green-600 text-lg sm:text-xl"></i>
                </div>
                <div class="min-w-0">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="px-5 sm:px-6 py-5 border-b border-slate-100">
                <h2 class="text-lg font-semibold text-slate-900">Daftar Slide</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ $slides->total() }} slide terdaftar</p>
            </div>
            ...
            <div class="p-5 sm:p-6">
                <form method="GET" action="{{ route('admin.hero-slides.index') }}" class="mb-6">
                    <div class="flex flex-col gap-3">
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}"
                                   placeholder="Cari judul atau subjudul slide..."
                            ...
                            @if(request('search'))
                                <a href="{{ route('admin.hero-slides.index', request()->except('search', 'page')) }}"
                    ...
                    @if($hasFilters)
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            ...
                            @endif
                            @if(request('status'))
                                <span class="inline-flex items-center h-7 px-2.5 rounded-lg bg-green-50 text-green-700 text-xs font-medium border border-green-100">
                                    {{ ucfirst(request('status')) }}
                                </span>
                            @endif
                            <span class="text-xs text-slate-400 sm:ml-auto">{{ $slides->total() }} hasil</span>
                        </div>
                    @endif
                </form>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50/80">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Gambar</th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-slate-800">{{ $slide->urutan }}</span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <img src="{{ $slide->gambar_url }}" alt="{{ $slide->judul ?: 'Slide' }}"
                                         class="w-28 h-16 object-cover rounded-lg border border-slate-200">
                                </td>
                                <td class="px-4 py-4">
                                    <p class="text-sm font-semibold text-slate-900">{{ $slide->judul ?: '(Tanpa judul)' }}</p>
                                    @if($slide->subjudul)
                                        <p class="text-sm text-slate-500 mt-0.5 max-w-xs line-clamp-2">{{ Str::limit($slide->subjudul, 60) }}</p>
                                    @endif
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-slate-900 mb-1">
                                        {{ $hasFilters ? 'Tidak ada slide yang cocok' : 'Belum ada slide' }}
                                    </h3>
                                    <p class="text-sm text-slate-500 mb-5">
                                        {{ $hasFilters ? 'Coba ubah kata kunci atau reset filter.' : 'Tambahkan gambar untuk ditampilkan di beranda.' }}
                                    </p>
                                    @if($hasFilters)
                                        <a href="{{ route('admin.hero-slides.index') }}"
                ...
                <div class="mt-5 pt-5 border-t border-slate-100">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <p class="text-sm text-slate-500">
                            Menampilkan
                            <span class="font-semibold text-slate-800">{{ $slides->firstItem() ?? 0 }}</span>
                            –
                            <span class="font-semibold text-slate-800">{{ $slides->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-semibold text-slate-800">{{ $slides->total() }}</span>
                        </p>
