...
@section('title', 'Edit Berita - ' . $schoolName)
...
@section('content')
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-6 text-white mb-6 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-blue-100">Edit berita yang sudah ada</p>
                    </div>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl">
                <div class="p-6">
                    <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="beritaForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="konten" 
                                      ...
                                      class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 textarea-konten">{{ old('konten', $berita->konten) }}</textarea>
                            @error('konten')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                        ...
                                        @foreach($kategori as $kat)
                                            <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors duration-200">
                                                <input type="checkbox" 
                                                       name="kategori_id[]" 
                                                       value="{{ $kat->id }}"
                                                       {{ in_array($kat->id, old('kategori_id', $berita->kategori->pluck('id')->toArray())) ? 'checked' : '' }}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function tampilkanTextareaFallback() {
                if (!isiTextarea) {
                    return;
                }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (gridContainer) {
                        ...
                        newCheckbox.innerHTML = '<input type="checkbox" name="kategori_id[]" value="' + data.kategori.id + '" checked class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 mr-3"><div class="flex items-center"><span class="text-sm text-gray-700 font-medium">' + data.kategori.nama + '</span></div>';
