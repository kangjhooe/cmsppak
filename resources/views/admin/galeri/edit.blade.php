...
@section('title', 'Edit Item Galeri - ' . $schoolName)
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-8 text-white mb-8 shadow-2xl relative overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div>
                                    <h1 class="text-3xl font-bold mb-1">Edit Galeri</h1>
                                    <p class="text-indigo-100 text-lg">Edit galeri dengan multiple media (foto/video/YouTube)</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-4 text-sm text-indigo-100">
                                <div class="flex items-center">
                                    <i class="fas fa-check-circle mr-2"></i>
                                    <span>Edit Multiple Media</span>
                                </div>
                                <div class="flex items-center">
                                    <i class="fas fa-check-circle mr-2"></i>
            ...
            <div class="bg-white overflow-hidden shadow-2xl rounded-2xl border border-gray-100">
                <div class="p-8">
                    <form action="{{ route('admin.galeri.update', $galeri) }}" method="POST" enctype="multipart/form-data" class="space-y-10" id="galeriForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border border-gray-200 shadow-sm">
                            <div class="flex items-center mb-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm">Edit informasi dasar galeri Anda</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <select name="status" 
                                        ...
                                        <option value="active" {{ old('status', $galeri->status) == 'active' ? 'selected' : '' }}>🟢 Active - Tampilkan di website</option>
                                        <option value="inactive" {{ old('status', $galeri->status) == 'inactive' ? 'selected' : '' }}>🔴 Inactive - Sembunyikan sementara</option>
                                    </select>
                                    @error('status')
                                        <div class="mt-3 flex items-center text-red-600 bg-red-50 px-4 py-2 rounded-lg border border-red-200">
                                            <i class="fas fa-exclamation-circle mr-2"></i>
                                ...
                                <div class="lg:col-span-2">
                                    ...
                                    <textarea name="deskripsi" 
                                              ...
                                              placeholder="Masukkan deskripsi galeri yang menjelaskan isi dan tujuan galeri ini (opsional)">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                            ...
                            @if($galeri->items && $galeri->items->count() > 0)
                                <div class="mb-8">
                                    <h4 class="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                                        ...
                                        Media Items Saat Ini ({{ $galeri->items->count() }})
                                    </h4>
                                            ...
                                            <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                                                <div class="flex items-center justify-between mb-3">
                                                    <div class="flex items-center">
                                                        ...
                                                        <div>
                                                            <h5 class="font-semibold text-gray-800 text-sm">{{ $item->judul }}</h5>
                                                            <p class="text-xs text-gray-500">{{ ucfirst($item->jenis) }}</p>
                                                        </div>
                                                    </div>
                                                    <div class="flex items-center space-x-2">
                                                        <span class="text-xs bg-gray-100 px-2 py-1 rounded-full">{{ $item->urutan }}</span>
                                                        <span class="text-xs {{ $item->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} px-2 py-1 rounded-full">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    @else
                                                        <div class="w-full h-24 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex items-center justify-center">
                ...
                <div class="bg-gray-50 px-8 py-6 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-600">
                            ...
                            <span>Ini adalah preview dari galeri yang akan Anda update</span>
                        </div>
    ...
    <script>
        ...
        function generateMediaPreview() {
            ...
            existingItems.forEach((item, index) => {
                const title = item.querySelector('h5')?.textContent || `Media ${index + 1}`;
                const type = item.querySelector('p.text-xs.text-gray-500')?.textContent || 'Media';
                const previewImg = item.querySelector('img');
                const youtubeIcon = item.querySelector('.fab.fa-youtube');
                
                if (previewImg) {
                    preview += `
                        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                            <div class="w-full h-24 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg flex items-center justify-center mb-2 overflow-hidden">
                                <img src="${previewImg.src}" class="w-full h-full object-cover rounded-lg" alt="${title}">
                            </div>
                    ...
                    preview += `
                        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                            <div class="w-full h-24 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex items-center justify-center mb-2">
                                <i class="fas fa-file text-gray-400 text-2xl"></i>
                            </div>
                            <p class="text-sm font-medium text-gray-800 truncate">${title}</p>
                            <p class="text-xs text-gray-500">${type}</p>
